...
class Appr(Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=100, lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
                 momentum=0, wd=0, multi_softmax=False, wu_nepochs=0, wu_lr_factor=1, logger=None,
    ...
    def extra_parser(args):
        ...
        parser.add_argument('--task_freeze_features', default=-1, type=int, required=False,
                            help='train Joint until specified task, then freeze features and continue Joint'
                                 '(-1: normal Joint, no freeze) (default=%(default)s)')
        return parser.parse_known_args(args)

    # Remove the warm-up phase since is not tested if it would work
    def pre_train_process(self, t, trn_loader):
        # Warm-up phase
        if self.warmup_epochs:
            print('Warning: warm-up phase is not available for Joint.')
