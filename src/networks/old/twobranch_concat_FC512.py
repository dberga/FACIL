...
class twobranch_concat_FC512(twobranch_concat):
    def __init__(self, pretrained=False, backbone='resnet18',num_out=100, togray="mean",scramble=False,select_kernels='all', remove_batchnorm=None):
        ...
        #twobranch init
        super(twobranch_concat_FC512, self).__init__(pretrained=pretrained,backbone=backbone,num_out=num_out,togray=togray,scramble=scramble,remove_batchnorm=remove_batchnorm,select_kernels=select_kernels)
        ...
        self.fc=torch.nn.Linear(in_features=512+512, out_features=num_out, bias=True) 
    '''
